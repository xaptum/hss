...
static void xarpcd_read_msg_callback( struct urb *urb )
{
	...
	if(!urb)
	{
		printk(KERN_ERR "xarpcd_usb: read_msg_callback with urb=NULL");
		goto exit;
	}
	if(urb->status != 0 )
	{
...
int xarpcd_send_msg( struct psock_proxy_msg *msg )
{
	...
	packet = usb_alloc_coherent(dev->udev, sizeof(psock_proxy_msg_packet_t)+msg->length-sizeof(psock_proxy_msg_t), GFP_KERNEL,
				 &urb->transfer_dma);
	if (!packet) {
	...
	packen_len = psock_proxy_msg_to_packet(msg,packet);
	if ( msg->length > sizeof( struct psock_proxy_msg ))
	{
