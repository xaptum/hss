...
uint32_t psock_proxy_msg_to_packet(psock_proxy_msg_t *msg, psock_proxy_msg_packet_t *packet)
{
    ...
    if(!packet)
    {
        printk( KERN_ERR KERN_DEBUG "WARNING: packet is null, exiting");
    }
    else if(!msg)
    {
        printk( KERN_ERR KERN_DEBUG "WARNING: msg is null, exiting");
    }
    else
    {
        packet_len = msg->length - sizeof(psock_proxy_msg_t) + sizeof(psock_proxy_msg_packet_t);
